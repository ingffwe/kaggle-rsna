...
class RSNADataset(Dataset):
    ...
    def __init__(self, subset='train', df_table=train_df, transform=None):
        ...
        sh_paths = glob(os.path.join('../input/final-second/train_volumes', "*.pt"))
        ...
        self.volume_dir2 = '../input/final-second/train_volumes'  # >1000 patient
...
for epoch in range(N_EPOCHS):
    ...
    if (val_loss_acc / valid_count) < best_val_loss:
        ...
        }, f"../output/ckpt-ckpt-3d-final/Conv3D_2_loss{best_val_loss:.4f}.pth")

        traced = torch.jit.trace(model.cpu(), torch.rand(1, 1, 224, 224, 224))
        traced.save(f'../output/ckpt-3d-final/Conv3D_2_jit_loss{best_val_loss:.4f}.pth')
    else:
        patience_counter += 1
