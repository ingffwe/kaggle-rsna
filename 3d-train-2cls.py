...
if AUGMENTATIONS:
    augs = transforms.Compose([
        augmentation.RandomRotation3D((10,10,30), resample='bilinear', p=0.5, same_on_batch=False, keepdim=True),
...
# # Load checkpoint
# PATH='../output/ckpt-3d-2cls/Conv3D_2_acc0.6139.pth'
...
scheduler = torch.optim.lr_scheduler.OneCycleLR(optimiser, max_lr=5e-5, epochs=5,
...
for epoch in range(N_EPOCHS):
    ...
    with torch.no_grad():
        ...
        for val_imgs, val_labels in tqdm(valid_loader):
            ...
            val_L = loss_function(val_preds, val_labels)

            value = torch.eq(val_preds.argmax(dim=1), val_labels)
    ...
    if (epoch + 1) % 1 == 0:
        print(
            f'Epoch {epoch + 1}/{N_EPOCHS}, loss {loss_acc / train_count:.5f}, val_loss {val_loss_acc / valid_count:.5f} , acc{metric:.5f},lr{scheduler.get_last_lr()[0]}')
    ...
    if (val_loss_acc / valid_count) < best_val_loss:
        ...
        }, f"../output/ckpt-3d-2cls/Conv3D_2_loss{best_val_loss:.4f}.pth")

        traced = torch.jit.trace(model.cpu(), torch.rand(1, 1, 224, 224, 224))
        traced.save(f'../output/ckpt-3d-2cls/Conv3D_2_loss{best_val_loss:.4f}_jit.pth')
    if metric > best_acc:
        best_acc = metric
        patience_counter = 0
        print('Valid acc improved --> saving model')
        torch.save({
            'epoch': epoch + 1,
            'model_state_dict': model.state_dict(),
            'optimiser_state_dict': optimiser.state_dict(),
            ...
            'loss': loss_acc / train_count,
            'val_loss': val_loss_acc / valid_count,
        }, f"../output/ckpt-3d-2cls/Conv3D_2_acc{metric:.4f}.pth")

        traced = torch.jit.trace(model.cpu(), torch.rand(1, 1, 224, 224, 224))
        traced.save(f'../output/ckpt-3d-2cls/Conv3D_2_acc{metric:.4f}_jit.pth')
    else:
        patience_counter += 1
