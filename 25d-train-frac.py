...
def train(train_loader, model, optimizer):
    ...
    best_loss = 100.0
    for batch_idx, (X, y_frac,_) in pbar:
        ...
        if batch_idx % 1000==0 and batch_idx!=0:
            ...
            if new_loss < best_loss:
                traced = torch.jit.trace(model.cpu(), torch.rand(1, 5, 384, 384))
                traced.save(f'../output/ckpt-25d-frac/ENb5_384_fold0_loss{new_loss:.4f}.pth')
                model = model.to('cuda')
                gc.collect()
                # torch.save(model.state_dict(), f'../output/ckpt-25d-frac/ENV2_384_fold0_loss{new_loss:.4f}.tph')
                best_loss = new_loss
            print({'loss': (frac_loss.item()), 'frac_loss': frac_loss.item(),
...
def validate(val_loader, model):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (X, y_frac, y_vert) in pbar:
            ...
            if batch_idx >= PREDICT_MAX_BATCHES:
                frac_loss = np.mean(frac_losses)
                return np.mean(frac_losses)

        frac_loss = np.mean(frac_losses)
    print("Val loss :"+ str(frac_loss))
    return frac_loss
...
if __name__ == '__main__':
    ...
    train_loader = torch.utils.data.DataLoader(ds_train, batch_size=16, shuffle=True, num_workers=8)
    val_loader = torch.utils.data.DataLoader(ds_val, batch_size=32, shuffle=False, num_workers=8)
    ...
    # model = torch.jit.load('../output/ckpt-25d-frac/ENV2_384_fold0_loss0.0494.pth')
    model = model.to('cuda')
    ...
    for _ in range(5):
        ...
        if val_loss < best_val_loss:
            traced = torch.jit.trace(model.cpu(), torch.rand(1, 5, 384, 384))
            traced.save(f'../output/ckpt-25d-frac/ENb5_384_fold0_val{val_loss:.4f}.pth')
            model = model.to('cuda')
