...
def main(cfg):
    ...
    print(f"run fold {cfg.fold}, train len: {len(train_dataset)} val len: {len(val_dataset)}")
                ...
                if cfg.load_best_weights is True:
                    try:
                        model.load_state_dict(
                            torch.load(f"{cfg.output_dir}/fold{cfg.fold}/{best_weights_name}.pth")["model"])
...
def run_eval(model, val_dataloader, post_pred, metric_function, seg_loss_func, cfg, epoch):
    ...
    with torch.no_grad():
        for itr in progress_bar:
            ...
            if cfg.run_tta_val is True:
                tta_ct = 1
                for dims in [[2], [3], [2, 3]]:
                    flip_val_outputs = sliding_window_inference(torch.flip(val_inputs, dims=dims), cfg.roi_size,
                                                                cfg.sw_batch_size, model)
            ...
            val_outputs = torch.stack(val_outputs)[:, 1:]
            ...
            if cfg.finetune_lb != -1:
                val_outputs = val_outputs[:, cfg.finetune_lb: cfg.finetune_lb + 1, ...]
                val_masks = val_masks[:, cfg.finetune_lb: cfg.finetune_lb + 1, ...]
    ...
    print(f"dice_score: {dice_score} hausdorff_score: {hausdorff_score}")
    torch.cuda.empty_cache()

    return dice_score * 0.4 + hausdorff_score * 0.6
...
def run_org_eval(model, val_dataloader, post_pred, metric_function, seg_loss_func, cfg, epoch):
    ...
    score = dice_metric.aggregate().item() * 0.4 + hausdorff_metric.aggregate().item() * 0.6
    dice_metric.reset()
    hausdorff_metric.reset()
...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--config", default="cfg_unet_multiclass", help="config filename")
